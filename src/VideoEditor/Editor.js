...
function Editor({videoUrl, timings, setTimings}) {
	...
	const [seekerBar, setSeekerBar] = useState(1)

	//State handling storing of the trimmed video
	const [trimmedVideo, setTrimmedVideo] = useState()

	//Boolean to handle whether the lottie animation is playing or not
	...
	//Ref handling the lottie's container
	...
	//Function handling trimming a videos duration given the UI's start and end time markers placement,
	//through ffmpeg command line operations, asynchronously
		...
		<div className='wrapper'>
			...
			<div className='controls'>
				...
				<div>
					...
					{!lottiePlaying ?
						<>
							<button title='Save changes' className='trim-control' onClick={() => {
								trimVideo()
						...
						:
						//Progress bar for filter application from progress-bar-react-ui library
						<ProgressBar
							progress={progress}
							width={300}
							color='linear-gradient(whitesmoke, #ED2290,black)'
							backgroundColor='linear-gradient(whitesmoke, whitesmoke,black)'
